#!/usr/bin/env bash
# ============================================================
# SAF (Stable Advantage Fusion) — OPD-only dedicated configuration (Mathematical Reasoning)
#
# The paper's OPD-only baseline uses a dedicated distillation configuration
# (larger batch size, n=1 rollout per prompt) rather than the shared
# GRPO/GRPO+OPD/SAF configuration in run_qwen3-4b-saf-opd-mixed.sh.
# See paper: "OPD-only uses a dedicated distillation configuration, while the
# other methods share a separate configuration."
#
...
#
# Usage examples:
#   TRAIN_MODE=opd  bash run_qwen3-4b-saf-opd-mixed-for-opd.sh
#   TRAIN_MODE=grpo bash run_qwen3-4b-saf-opd-mixed-for-opd.sh
# ============================================================
...
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"checkpoints/saf-opd-math-opd-config"}
...
python3 -m verl.trainer.main_ppo \
        ...
        "trainer.experiment_name=saf_opd_math_opd_config_${experiment_suffix}" \
