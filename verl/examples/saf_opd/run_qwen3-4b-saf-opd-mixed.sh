#!/usr/bin/env bash
# ============================================================
# SAF (Stable Advantage Fusion) — Mathematical Reasoning
#
# Reproduces the GRPO / OPD-only / GRPO+OPD(fixed) / SAF settings from the
# paper "SAF: Stable Advantage Fusion for On-Policy Distillation" on
# mathematical-reasoning data (DeepMath, difficulty >= 6).
#
...
#
# Usage examples:
#   TRAIN_MODE=grpo bash run_qwen3-4b-saf-opd-mixed.sh
#   TRAIN_MODE=opd  bash run_qwen3-4b-saf-opd-mixed.sh
#   TRAIN_MODE=mixed GRPO_COEF=1.0 OPD_COEF=1.0 bash run_qwen3-4b-saf-opd-mixed.sh   # fixed fusion
#   TRAIN_MODE=mixed TOPK_PERCENT=20 TANH_COMPRESS=True TANH_COEF=0.1 \
#     OPD_WARMUP_STEPS=100 KL_DROP_DELTA=0.2 OPD_ANNEAL=True \
#     bash run_qwen3-4b-saf-opd-mixed.sh                                             # full SAF
# ============================================================
...
# Base student model and teacher model checkpoints (HuggingFace format).
STUDENT_MODEL_PATH=${STUDENT_MODEL_PATH:-"Qwen/Qwen3-8B"}
TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-"Qwen/Qwen3-30B-A3B-Instruct-2507"}

# Training data: DeepMath-103K filtered to difficulty >= 6 (see paper Sec. Experimental Setup).
TRAIN_FILE=${TRAIN_FILE:-"data/deepmath/train_filtered_level6.parquet"}
# Validation data: AIME24 / AIME25 (comma-separated list of parquet files).
AIME24_TEST_PATH=${AIME24_TEST_PATH:-"data/aime24/test.parquet"}
AIME25_TEST_PATH=${AIME25_TEST_PATH:-"data/aime25/test.parquet"}
test_files="['$AIME24_TEST_PATH', '$AIME25_TEST_PATH']"

CHECKPOINT_DIR=${CHECKPOINT_DIR:-"checkpoints/saf-opd-math"}
...
# Keep only the top-TOPK_PERCENT% OPD tokens by |A_opd| per sequence; rest are zeroed.
# 100 = no filtering (default). Paper default: 20.
TOPK_PERCENT=${TOPK_PERCENT:-100}

# ---- SAF Stage 2: bounded tanh compression ----
# A_opd_tanh = TANH_COEF * tanh(A_opd), confines the OPD advantage to (-TANH_COEF, TANH_COEF).
TANH_COMPRESS=${TANH_COMPRESS:-False}
TANH_COEF=${TANH_COEF:-0.1}

# ---- SAF Stage 3: KL-triggered linear warm-up ----
# OPD_WARMUP_STEPS=N : linearly ramp the OPD coefficient from 0 to 1 over the first N steps.
#   0 = no warm-up (default); also disables Stage 4.
# KL_DROP_DELTA : early-stop warm-up once (KL_0 - KL_t) / KL_0 >= delta, e.g. 0.2 = 20% drop.
#   0 = disabled (warm-up always runs the full OPD_WARMUP_STEPS).
...
# Activates the step after warm-up ends (or from step 0 if warm-up is disabled).
# opd_coef(s) = ANNEAL_MIN + (1 - ANNEAL_MIN) * (1 - t / R)
# ANNEAL_TOTAL_STEPS (optional): total steps for warm-up + annealing combined; the
#   remaining annealing budget R is then computed as max(1, ANNEAL_TOTAL_STEPS - warmup_end_step),
#   taking precedence over ANNEAL_STEPS.
...
python3 -m verl.trainer.main_ppo \
        ...
        "trainer.experiment_name=saf_opd_math_${experiment_suffix}" \
