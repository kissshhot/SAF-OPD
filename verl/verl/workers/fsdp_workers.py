...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_ref_log_prob(self, data: DataProto):
        if self._is_lora:
            ...
            if "entropys" in data.batch and data.batch["entropys"] is not None:
                tensors["ref_entropy"] = data.batch["entropys"]
        ...
        with self.ulysses_sharding_manager:
            ...
            output, ref_entropy = self.ref_policy.compute_log_prob(data=data, calculate_entropy=True)
            tensors = {"ref_log_prob": output}
            if ref_entropy is not None:
                tensors["ref_entropy"] = ref_entropy
