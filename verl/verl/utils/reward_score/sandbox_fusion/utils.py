...
try:
    from pssdk import BaseSandboxClusterTool, with_retry as pssdk_with_retry

    class _PssdkSandboxTool(BaseSandboxClusterTool):
        def submit_code_async(self, code, language, compile_timeout, run_timeout, stdin=None, task_timeout=600):
            body = {
                "code": code, "language": language,
                "compile_timeout": compile_timeout, "run_timeout": run_timeout,
                "stdin": stdin, "files": {}, "fetch_files": [], "stdin_list": None,
            }
            single_request = {"body": body, "params": {}, "headers": {}, "call_timeout": 0}
        ...
        def get_async_result(self, task_id, call_timeout=10):
            return self.async_get(task_id=task_id, call_timeout=call_timeout)

    _PSSDK_AVAILABLE = True
except ImportError:
    _PSSDK_AVAILABLE = False

SANDBOX_CLUSTER_ENDPOINT = os.getenv("SANDBOX_CLUSTER_ENDPOINT", "")
...
def call_sandbox_api(
    ...
    """
    Calls the sandbox API to execute code.
    Uses pssdk (company sandbox) when SANDBOX_CLUSTER_ENDPOINT/SANDBOX_TOKEN are set,
    otherwise falls back to direct HTTP POST to sandbox_fusion_url.
    """
    endpoint = SANDBOX_CLUSTER_ENDPOINT or sandbox_fusion_url
    token = SANDBOX_TOKEN

    if _PSSDK_AVAILABLE and endpoint and token:
        try:
            tool = _PssdkSandboxTool(endpoint, token)
            task_timeout = compile_timeout + run_timeout + 60
            submit_result = tool.submit_code_async(
                code=code, language=language,
                compile_timeout=compile_timeout, run_timeout=run_timeout,
                stdin=stdin, task_timeout=task_timeout,
            )
            task_id = submit_result.get("taskId")
            if not task_id:
                return None, f"pssdk async submit failed: no taskId in response: {submit_result}"

            poll_interval = 0.1
            max_poll_interval = 1.0
            deadline = time.time() + task_timeout
            while time.time() < deadline:
                poll_result = tool.get_async_result(task_id)
                if poll_result.get("complete"):
                    requests_list = poll_result.get("requests", [])
                    if requests_list:
                        result_str = requests_list[0].get("result")
                        if result_str:
                            return json.loads(result_str), None
                    return None, f"async complete but no result: {poll_result}"
                time.sleep(poll_interval)
                poll_interval = min(poll_interval * 1.5, max_poll_interval)

            return None, f"pssdk async poll timeout after {task_timeout}s"
        except Exception as e:
            return None, f"pssdk sandbox error: {e}"

    # Fallback: direct HTTP POST (original sandbox fusion protocol)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response = requests.post(sandbox_fusion_url, headers=headers, data=payload, timeout=request_timeout)
