...
def compute_score(
    ...
    """
    if strict_box_verify:
        correct_int, pred = is_correct_strict_box(solution_str, ground_truth, pause_tokens_index)
        correct = correct_int == 1
    else:
        # Extract answer directly from \boxed{} to avoid "Answer:\n\n$$\n\boxed{X}" format issues
        # where the regex-based is_correct_minerva captures "$$" instead of the actual answer.
        boxed_str = last_boxed_only_string(solution_str)
        pred = remove_boxed(boxed_str) if boxed_str is not None else ""
        if pred:
            from math_verify import parse as mv_parse, verify as mv_verify
            try:
                correct = mv_verify(
                    mv_parse(f"\\boxed{{{ground_truth}}}"),
